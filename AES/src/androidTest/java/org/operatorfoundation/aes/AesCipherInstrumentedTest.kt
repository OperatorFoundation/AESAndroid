...
class AesCipherInstrumentedTest {
    ...
    @Test
    fun `encrypt and decrypt work across process death simulation`() {
        val key = AesGcmKey.generate()
        val plaintext = "test data".toByteArray()

        val ciphertext = cipher.encrypt(key, plaintext)

        // Serialize to simulate saving/loading
        val keyBytes = key.bytes
        val nonceBytes = ciphertext.nonce.bytes
        val encryptedBytes = ciphertext.encrypted
        val tagBytes = ciphertext.tag

        // Reconstruct as if loaded from storage
        val restoredKey = AesGcmKey(keyBytes)
        val restoredCiphertext = Ciphertext(
            Nonce(nonceBytes),
            encryptedBytes,
            tagBytes
        )

        val decrypted = cipher.decrypt(restoredKey, restoredCiphertext)
        assertArrayEquals(plaintext, decrypted)
    }

    @Test
    fun `SecureRandom produces quality randomness on device`() {
        val key1 = AesGcmKey.generate()
        val key2 = AesGcmKey.generate()
        ...
        // Check for all-zeros or obvious patterns (shouldn't happen with good RNG)
        assertFalse(key1.bytes.all { it == 0.toByte() })
        assertFalse(key2.bytes.all { it == 0.toByte() })
    }

    @Test
    fun `encryption performance is acceptable on device`() {
        val key = AesGcmKey.generate()
        ...
        // Sanity check - should be well under 100ms on any modern device
